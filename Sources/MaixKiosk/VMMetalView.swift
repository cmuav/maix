...
class VMMetalView: MTKView {
    ...
    private let isHandleInitialClick: Bool = false
    private let isCtrlCmdSwapped: Bool = false
    private let isISOKeySwapped: Bool = false
    ...
    override func updateTrackingAreas() {
        ...
        NSLog("update tracking area: \(trackingArea.rect)")
        if let oldTrackingArea = wholeTrackingArea {
            NSLog("remove old tracking area: \(oldTrackingArea.rect)")
    ...
    override func mouseEntered(with event: NSEvent) {
        NSLog("mouse entered (first responder: \(isFirstResponder))")
    ...
    override func mouseMoved(with event: NSEvent) {
        NSLog("mouse moved: \(event.deltaX), \(event.deltaY)")
        if isMouseCaptured {
...
extension VMMetalView {
    private var screenCenter: CGPoint? {
        ...
        NSLog("screen \(centerScreen.x), \(centerScreen.y)")
        return centerScreen
    }
    
    func captureMouse() {
        NSLog("capture cursor")
